...
class RecommendationGenerator:
    ...
    def generate_recommendations(self,
        ...
        if commit.get('tests_added', 0) == 0 and has_bug_keyword:
            recommendations.append(
                "Для исправления бага не добавлены тесты. Добавьте тесты для проверки исправления.")
        ...
        if median_lines_author is not None and lines_added > 2 * median_lines_author:
            recommendations.append(
                f"👤 Автор {author} внёс {lines_added} строк — это более чем в 2 раза выше его обычного объёма. Рекомендуется код ревью."
            )
